...
Exit non-zero in strict mode so CI/pre-commit blocks PRs.
...
def main():
    ...
    ap.add_argument("--strict", action="store_true", help="Exit non-zero when findings present")
    args = ap.parse_args()
    findings = scan(Path(args.root))
    if not findings:
        print("No disallowed cloud SDK imports found.")
        return 0
    print("Disallowed direct cloud SDK imports found (replace with StorageClient/loader):")
    for p, ln, txt in findings:
        print(f"{p}:{ln}: {txt}")
    if args.strict:
        print("\nStrict mode: failing with non-zero exit code.")
        return 2
    return 0
