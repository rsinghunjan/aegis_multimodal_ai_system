...
#!/usr/bin/env python3
"""
Safe auto-refactor helper to produce .refactored.py candidate files for review.

It flags/badges common direct cloud SDK usage and annotates lines with suggested replacement.
It does NOT modify originals; it writes <orig>.refactored.py for manual review.
...
RULES = [
    ...
    (re.compile(r"(^\s*import\s+boto3\s*$)", re.MULTILINE), "# REPLACE: remove direct boto3 import; use storage.factory"),
    ...
    (re.compile(r"(^\s*import\s+oci.+$)", re.MULTILINE), "# REPLACE: use storage.factory or adapter"),
]
