...
import asyncio
from aegis_multimodal_ai_system.agentic.agent_manager import AgentManager, example_echo_tool, async_wrapper_example


def test_sync_tool_invocation():
    am = AgentManager()
    am.register_tool("echo", example_echo_tool)
    res = asyncio.run(am.dispatch_action({"tool": "echo", "payload": {"text": "hello"}}))
...
def test_async_tool_invocation():
    ...
    res = asyncio.run(am.dispatch_action({"tool": "async_echo", "payload": {"text": "async"}}))
    assert res["status"] == "ok"
...
def test_timeout_behavior():
    ...
    res = asyncio.run(am.dispatch_action({"tool": "slow", "timeout_seconds": 0.01}))
    assert res["status"] == "error"
